...
class imagenet(datasets.imdb):
	...
	if not os.path.exists(filename):
		...
		data_sum = None
		idx = 0
		#flist = ['/home/dev/download/opensource/py-faster-rcnn/data/ILSVRC2015_DET_sample/data/n02769748.mat','/home/dev/download/opensource/py-faster-rcnn/data/ILSVRC2015_DET_sample/data/n02958343.mat']
		#for sub_file in flist:
		print os.path.join(self._devkit_path,'data/*.mat')
		for sub_file in glob(os.path.join(self._devkit_path,
                                                 'data/*.mat')):
			print "XX:",sub_file
			idx += 1
			each_data = sio.loadmat(sub_file)['all_boxes'][0]
	
			if idx == 1:
				data_sum = each_data
			else:
				data_sum = np.concatenate((data_sum,each_data), axis=0)
	        box_list = []
        	for i in xrange(data_sum.shape[0]):
            		box_list.append(data_sum[i][:, (1, 0, 3, 2)])
